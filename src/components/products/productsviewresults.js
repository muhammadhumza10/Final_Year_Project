...
export default function ProductsView({ page, limitPerPage,setCount }) {
  ...
  const {searchquery, fabric_type, qual, sort_by } = router.query;
  ...
  const getProducts = async (page) => {
    let a = await fetch(`/api/getresults`, {
      method: "POST", // or 'PUT'
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({ page, limitPerPage,searchquery, fabric_type, qual, sort_by }),
    });
  ...
  const clearfilters = () => {
    ...
    setQuality("");
    setSortby("");
    router.push(`/buyer/results?searchquery=${searchquery}`);
  };
  ...
  const applysearch = () => {
    if (fabrictype) {
      if (quality) {
        if (sortby) {
          router.push(
            `/buyer/results?searchquery=${searchquery}&fabric_type=${fabrictype}&qual=${quality}&sort_by=${sortby}`
          );
        } else {
          router.push(
            `/buyer/results?searchquery=${searchquery}&fabric_type=${fabrictype}&qual=${quality}`
          );
        }
      } else if (sortby) {
        router.push(
          `/buyer/results?searchquery=${searchquery}&fabric_type=${fabrictype}&sort_by=${sortby}`
        );
      } else {
        router.push(`/buyer/results?searchquery=${searchquery}&fabric_type=${fabrictype}`);
      }
    } else if (quality) {
      if (sortby) {
        router.push(`/buyer/results?searchquery=${searchquery}&qual=${quality}&sort_by=${sortby}`);
      } else {
        router.push(`/buyer/results?searchquery=${searchquery}&qual=${quality}`);
      }
    } else if (sortby) {
      router.push(`/buyer/results?searchquery=${searchquery}&sort_by=${sortby}`);
    }
  };
