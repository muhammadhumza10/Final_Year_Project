...
export default function Id({product,user,logout,placeOrder,productratingscount,ratings}) {
 ...
 <ProductDetailsCarousel product={product}/>
        ...
        <div className="max-w-2xl mx-auto pt-10 pb-16 px-4 sm:px-6 lg:max-w-7xl lg:pt-16 lg:pb-24 lg:px-8 lg:grid lg:grid-cols-3 lg:grid-rows-[auto,auto,1fr] lg:gap-x-8">
          ...
          <div className="mt-4 lg:mt-0 lg:row-span-3">
            ...
            <form className="mt-10">
              ...
              <div className="flex flex-col">
                ...
                <Typography variant="h4" sx={{ml:2,fontWeight:600}} >{product.color}</Typography>
                </div>
               
                <div className='flex flex-row mt-4 items-center'>
              <Typography variant="h4" >Fabric Type : </Typography>
                <Typography variant="h4" sx={{ml:2,fontWeight:600, textAlign:'center'}} >{product.fabrictype}</Typography>
              </div>
                <div className='flex flex-row mt-4 items-center'>
                <Typography variant="h4" >Quality : </Typography>
                <Typography variant="h4" sx={{ml:2,fontWeight:600, textAlign:'center'}} >{product.quality}</Typography>
                </div>
              <div className='flex flex-row mt-4 items-center'>
              <Typography variant="h4" >Minimum Order Quantity : </Typography>
                <Typography variant="h4" sx={{ml:2,fontWeight:600, textAlign:'center'}} >{product.moq}</Typography>
              </div>
                <div className='flex flex-row mt-4 items-center'>
                <Typography variant="h4" >Delivery Time : </Typography>
                <Typography variant="h4" sx={{ml:2,fontWeight:600, textAlign:'center'}} >{product.deliverytime}</Typography>
                </div>
              </div>
              ...
              <LoadingButton
                ...
                onClick={()=>{
                  setLoading(true)
                router.push(`/seller/myproducts/activeproducts/editproduct/${product._id}`)}}
              >
                Edit
              </LoadingButton>
            </form>
          ...
          <div className="py-10 lg:pt-6 lg:pb-16 lg:col-start-1 lg:col-span-2 lg:border-r lg:border-gray-200 lg:pr-8">
            ...
            <div>
              <Typography variant='h2' sx={{fontWeight:600}} >Description : </Typography>

              <div className="space-y-6 mt-5">
                
                <Markdown className='markdown' children={product.description}/>
              </div>
            </div>
...
<ProductDetailsReview productratingscount={productratingscount} ratings={ratings} />
</BaseCard>
</>
...
export async function getServerSideProps(context) {
  ...
  let productratingscount = await Rating.count({ "order.product._id": context.query._id })
