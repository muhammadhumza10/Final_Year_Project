...
export default function ProductDetailsCarousel({ product }) {
  ...
  const imagesLightbox = product.imageUrl.map((_image) => _image);

  const handleOpenLightbox = (img) => {
    const selectedImage = imagesLightbox.findIndex((index) => index === img);
  ...
  const settings2 = {
    ...
    variableWidth: true,
    centerPadding: '0px',
    slidesToShow: product.imageUrl.length > 3 ? 3 : product.imageUrl.length,
  };
    ...
    <RootStyle>
      ...
      <Box
        sx={{
          ...
          ...(product.imageUrl.length === 1 && { maxWidth: THUMB_SIZE * 1 + 16 }),
          ...(product.imageUrl.length === 2 && { maxWidth: THUMB_SIZE * 2 + 32 }),
          ...(product.imageUrl.length === 3 && { maxWidth: THUMB_SIZE * 3 + 48 }),
          ...(product.imageUrl.length === 4 && { maxWidth: THUMB_SIZE * 3 + 48 }),
          ...(product.imageUrl.length >= 5 && { maxWidth: THUMB_SIZE * 6 }),
