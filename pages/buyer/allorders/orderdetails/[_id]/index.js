...
import ProductDetailsReviewForm from "../../../../../src/components/reviews/ProductDetailsReviewForm";
...
export default function Example({ order, user, rating }) {
  ...
  const headName = `${
    order.product.productname.charAt(0).toUpperCase() +
    order.product.productname.slice(1)
  } - ${router.query._id} | ${COMPANY_NAME}`;
          ...
          <div className="text-sm border-b border-gray-200 mt-2 pb-5">
            ...
            <div className="mt-4">
              {order.isReviewed == true ? (
                <>
                  <div
                    className="p-2 w-max rounded-md flex items-center"
                ...
                <div>
                  ...
                  {order.status == "Completed" ? (
                    ...
                    <Typography
                      ...
                      sx={{ mt: 2 }}
                    >
                      You can write review once order is completed.
                    </Typography>
                  )}
                </div>
              )}
...
export async function getServerSideProps(context) {
  ...
  let rating = await Ratings.findOne({ "order._id": context.query._id });
