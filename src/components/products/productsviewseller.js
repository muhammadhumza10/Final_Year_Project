...
export default function ProductsViewSeller({ page, limitPerPage,setCount }) {
  const router = useRouter();
  const query = router.query;
  const { fabric_type, qual, sort_by, _id } = router.query;
  ...
  const getProducts = async (page) => {
    let a = await fetch(`/api/getallproductsseller`, {
      method: "POST", // or 'PUT'
      headers: {
  ...
  const clearfilters = () => {
    ...
    setQuality("");
    setSortby("");
    router.push(`/buyer/products/seller/${_id}`);
  };
  ...
  const applysearch = () => {
    if (fabrictype) {
      if (quality) {
        if (sortby) {
          router.push(
            `/buyer/products/seller/${_id}?fabric_type=${fabrictype}&qual=${quality}&sort_by=${sortby}`
          );
        } else {
          router.push(
            `/buyer/products/seller/${_id}?fabric_type=${fabrictype}&qual=${quality}`
          );
        }
      } else if (sortby) {
        router.push(
          `/buyer/products/seller/${_id}?fabric_type=${fabrictype}&sort_by=${sortby}`
        );
      } else {
        router.push(`/buyer/products/seller/${_id}?fabric_type=${fabrictype}`);
      }
    } else if (quality) {
      if (sortby) {
        router.push(
          `/buyer/products/seller/${_id}?qual=${quality}&sort_by=${sortby}`
        );
      } else {
        router.push(`/buyer/products/seller/${_id}?qual=${quality}`);
      }
    } else if (sortby) {
      router.push(`/buyer/products/seller/${_id}?sort_by=${sortby}`);
    }
  };
        ...
        <div className="px-4 mt-10">
          <div className="mt-4 flex flex-col">
            ...
            {fabrictype || quality || sortby ? (
              <Button sx={{ mt: 2 }} variant="contained" onClick={applysearch}>
                Apply Filters
              </Button>
            ) : (
